...
@wizard.route("/dashboard")
def dashboard_page():
    """Serve the pipeline dashboard HTML."""
    from flask import send_file
    return send_file(os.path.join(os.path.dirname(__file__), "static", "pipeline-dashboard.html"))


@wizard.route("/api/pipeline/status")
def pipeline_status():
    """
    Get comprehensive pipeline status for all companies.
    Returns all company data with their progress across all stages.
    """
    from ggplay.run_pipeline import _load_existing_results
    ...
    if not os.path.exists(EXCEL_FILE):
        return jsonify({
            ...
            "stages": {
                "search_details": {"completed": 0, "total": 0},
                "duns": {"completed": 0, "total": 0},
                "certificate": {"completed": 0, "total": 0},
                "domain": {"completed": 0, "total": 0},
                "email": {"completed": 0, "total": 0},
                "xbinder": {"completed": 0, "total": 0},
                "play_console": {"completed": 0, "total": 0},
                "apk": {"completed": 0, "total": 0},
            }
        })

    try:
        companies = _load_existing_results(EXCEL_FILE)
        
        # Calculate summary stats
        summary = {"success": 0, "pending": 0, "failed": 0, "total": len(companies)}
        stages = {
            "search_details": {"completed": 0, "total": len(companies)},
            "duns": {"completed": 0, "total": len(companies)},
            "certificate": {"completed": 0, "total": len(companies)},
            "domain": {"completed": 0, "total": len(companies)},
            "email": {"completed": 0, "total": len(companies)},
            "xbinder": {"completed": 0, "total": len(companies)},
            "play_console": {"completed": 0, "total": len(companies)},
            "apk": {"completed": 0, "total": len(companies)},
        }

        for company in companies:
            # Count stage completions
            if company.get("details", {}).get("company_name"):
                stages["search_details"]["completed"] += 1
            if company.get("duns", {}).get("duns_number"):
                stages["duns"]["completed"] += 1
            if company.get("certificate", {}).get("path"):
                stages["certificate"]["completed"] += 1
            if company.get("domain", {}).get("domain"):
                stages["domain"]["completed"] += 1
            if company.get("email", {}).get("email"):
                stages["email"]["completed"] += 1
            if company.get("representative_id", {}).get("status") == "downloaded":
                stages["xbinder"]["completed"] += 1
            if company.get("play_console", {}).get("developer_email"):
                stages["play_console"]["completed"] += 1
            if company.get("apk_generation", {}).get("status") == "success":
                stages["apk"]["completed"] += 1

            # Count overall status
            if company.get("apk_generation", {}).get("status") == "success":
                summary["success"] += 1
            elif any([
                company.get("certificate", {}).get("error"),
                company.get("domain", {}).get("error"),
                company.get("apk_generation", {}).get("error"),
            ]):
                summary["failed"] += 1
            else:
                summary["pending"] += 1
            ...
            # Add timestamp
            company["updated_at"] = company.get("updated_at", datetime.now().isoformat())
        ...
        import traceback
        traceback.print_exc()
        return jsonify({"error": str(e), "companies": [], "summary": {}}), 500
...
@wizard.route("/control")
def control_panel():
    from flask import send_file
    return send_file(os.path.join(os.path.dirname(os.path.abspath(__file__)), "static", "control.html"))


@wizard.route("/api/companies/dashboard")
def companies_dashboard():
    """Per-company stage completion for the control panel."""
    import re as _re
    ROOT = os.path.dirname(os.path.abspath(__file__))
    COMP_DIR = os.path.join(ROOT, "pipeline_output", "companies")

    if not os.path.exists(EXCEL_FILE):
        return jsonify({"companies": [], "summary": {"total": 0, "complete": 0, "partial": 0, "pending": 0}})
    ...
    try:
        ...
        cn_c = _c("company number"); name_c = _c("company name")
        cert_c = _c("certificate downloaded"); domain_c = _c("domain")
        email_c = _c("assigned email"); duns_c = _c("duns number")

        companies = []
        for row in ws.iter_rows(min_row=2, values_only=True):
            if not row or cn_c is None or not row[cn_c]:
                continue
            cn = str(row[cn_c]).strip().zfill(8)
            name = str(row[name_c] or "").strip() if name_c is not None else ""
            safe = _re.sub(r'[\\/:*?"<>|]', "", name).strip()
            folder = f"{cn} - {safe}" if safe else cn
            cdir = os.path.join(COMP_DIR, folder)

            duns   = str(row[duns_c]   or "").strip() if duns_c   is not None else ""
            domain = str(row[domain_c] or "").strip() if domain_c is not None else ""
            email  = str(row[email_c]  or "").strip() if email_c  is not None else ""

            has_cert = bool(row[cert_c] if cert_c is not None else False)
            if not has_cert:
                cert_d = os.path.join(cdir, "certificate")
                has_cert = os.path.isdir(cert_d) and any(f.endswith("_cert.pdf") for f in os.listdir(cert_d))

            has_id = os.path.exists(os.path.join(cdir, "director_id", "ID Front.jpeg"))

            art_dir = os.path.join(cdir, "app", "artifacts")
            has_app = os.path.isdir(art_dir) and any(
                f.endswith((".apk", ".aab")) for f in os.listdir(art_dir)
            )
            ...
            companies.append({
                "cn": cn, "name": name, "stages": stages,
                "n_done": sum(stages.values()), "n_total": len(stages),
        ...
        return jsonify({
            ...
            "summary": {
                "total":    n,
                "complete": sum(1 for c in companies if c["n_done"] == c["n_total"]),
                "partial":  sum(1 for c in companies if 0 < c["n_done"] < c["n_total"]),
                "pending":  sum(1 for c in companies if c["n_done"] == 0),
            },
        })
    except Exception as e:
